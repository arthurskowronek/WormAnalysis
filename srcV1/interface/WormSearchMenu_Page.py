...
class WormSearchMenu:
    ...
    def _create_buttons(self) -> Dict[str, Button]:
        ...
        buttons["Launch"] = Button(center_x, 1124, 400, 120, "Launch Scan", "primary")
        ...
        left_x = 640
        left_y_start = 1150
        buttons["DualView"] = Button(left_x, left_y_start, 180, 80, "Dual View", "danger")
        buttons["Shape_square"] = Button(left_x - 40, left_y_start + 110, 80, 80, "", "default")
        buttons["Shape_rectangle"] = Button(left_x + 60 , left_y_start + 110, 160, 80, "", "default")
        buttons["Time_exposure"] = Button(left_x, left_y_start + 220, 180, 80, "", "default")
    ...
    def draw_section_labels(self, img: np.ndarray):
        ...
        # Shape of the scan section
        cv2.putText(img, "Shape of the scan :", (360, 1310), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, self.theme.text_secondary, 2)
        
        # Time exposure section
        cv2.putText(img, "Time exposure (ms) :", (380, 1420), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, self.theme.text_secondary, 2)
        ...
        cv2.putText(img, "Text box (if you use another objective)", (scan_x+200, 1415),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, self.theme.text_secondary, 2)
        cv2.putText(img, "Validate input with 'enter'", (scan_x+280, 1445),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, self.theme.text_secondary, 2)
    ...
    def update_button_states(self):
        ...
        for name, button in self.buttons.items():
            if name.startswith("Scan_") and not name.endswith("_Input"):
                ...
                shape_value = name.split("_")[1]
                button.is_active = (self.shape == shape_value)
    ...
    def draw_interface(self) -> np.ndarray:
        ...
        cv2.putText(img, "Be sure to have the objective", (50, 620), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(img, "in the lower right corner", (85, 660), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(img, "and to use the L camera", (78, 700), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
